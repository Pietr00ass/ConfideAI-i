...
def get_current_user(request: Request) -> User:
    ...
    if not uid: raise HTTPException(401, "Zaloguj się")
    with Session(engine) as sess:
        u = sess.get(User, uid)
    if not u: raise HTTPException(401, "Nieprawidłowa sesja")
    return u
...
async def scan_upload(request: Request, file: UploadFile = File(...), user: User = Depends(get_current_user)):
    ...
    with Session(engine) as sess:
        ...
        for name, fn in [("email",detect_email),("pesel",detect_pesel),("cc",detect_cc),("ml",detect_ml)]:
            matches = fn(text) or []
...
def scan_detail(request: Request, job_id: int, user: User = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("scan/detail.html", {"request":request,"job":job,"results":results,"user":user})

@router.get("/history", response_class=HTMLResponse)
def scan_history(request: Request, user: User = Depends(get_current_user)):
    with Session(engine) as sess:
        jobs = sess.exec(select(ScanJob).where(ScanJob.user_id==user.id).order_by(ScanJob.created_at.desc())).all()
    return templates.TemplateResponse("scan/history.html", {"request":request,"jobs":jobs,"user":user})
